...
static const struct {
  ...
  const char* longName;
    ...
    {BUTTON_ZL, "ZL", "ZL"},
    {BUTTON_ZR, "ZR", "ZR"},
    {BUTTON_START, "St", "Start"},
    {BUTTON_SELECT, "Se", "Select"},
    {BUTTON_UP, "^", "Up"},
    {BUTTON_DOWN, "v", "Down"},
    {BUTTON_LEFT, "<", "Left"},
    {BUTTON_RIGHT, ">", "Right"},
    {CPAD_UP, "c^", "cp-up"},
    {CPAD_DOWN, "cv", "cp-down"},
    {CPAD_LEFT, "c<", "cp-left"},
    {CPAD_RIGHT, "c>", "cp-right"},
    {CSTICK_UP, "s^", "cs-up"},
    {CSTICK_DOWN, "sv", "cs-down"},
    {CSTICK_LEFT, "s<", "cs-left"},
    {CSTICK_RIGHT, "s>", "cs-right"},
};
...
void Commands_ShowCommands(void) {
  ...
  do {
    ...
    for (s32 i = 0;
      ...
      Draw_DrawCharacter(STRICT_COLUMN_X, 30 + i * SPACING_Y, COLOR_WHITE,
                         commandList[j].strict ? 'S' : 'R');
