...
namespace game {
  ...
  /// Re-point any button showing `previous` at `replacement`, but only if no
  /// bottle still holds `previous` -- with two bottles of the same contents the
  /// button is still valid and must be left alone. Call after updating bottles.
  static void RemapBottleButton(ItemId previous, ItemId replacement) {
    if (previous == ItemId::None || previous == replacement || HasBottle(previous))
      return;
    auto& equips = GetCommonData().save.equipment;
    // Skip index 0: that is B, which holds the sword rather than an item.
    for (size_t btn = 1; btn < equips.data[0].item_btns.size(); ++btn) {
      if (equips.data[0].item_btns[btn] == previous)
        equips.data[0].item_btns[btn] = replacement;
    }
  }

  void RemoveBottle(u32 bottle_index) {
    auto& bottles = GetCommonData().save.inventory.bottles;
    const ItemId previous = bottles[bottle_index];
    bottles[bottle_index] = ItemId::None;
    // Otherwise a button keeps showing contents no bottle holds any more.
    RemapBottleButton(previous, ItemId::None);
  }
  ...
  void GiveBottle(u32 bottle_index, ItemId item_id) {
    ...
    // Only follow a button that was actually showing this bottle. Filling an
    // empty slot used to match the first empty button -- both being None -- and
    // silently equip every newly given bottle to Y.
    RemapBottleButton(previous, item_id);
  }
  ...
  void GiveItemWithEffect(u32 gi_item_index) {
    ...
    rnd::util::GetPointer<int(game::GlobalContext*, game::act::Player*)>(ADDR_if_231038)(gctx, player);
  }
  ...
  void GiveItem(ItemId item_id) {
    ...
    rnd::util::GetPointer<void(game::ItemId)>(ADDR_GiveItem_22B14C)(item_id);
  ...
  bool CanUseItemUnsafe(ItemId item_id) {
    ...
    const auto update_usability = rnd::util::GetPointer<void(GlobalContext*)>(ADDR_if_1884B0);
  ...
  bool CanUseItem(ItemId item) {
    ...
    const auto get_item_usability_mode = rnd::util::GetPointer<int(GlobalContext*)>(ADDR_if_1E827C);
    ...
    // The item being allowed in the table is only a necessary condition for being allowed to use
    // the item; it is not a sufficient one.
