...
namespace msys {
  ...
  static void DisableMenuToggles(ToggleMenu* menu) {
    // nbItems, not nbItems - 1: the old bound left the last row set, which is the
    // "None" entry on the bottles menu.
  ...
  static void Inventory_ItemsMenuInit(void) {
    ...
    InventoryItemsMenu.items[(u32)game::ItemId::Bombchu - 1].on = game::HasItem(game::ItemId::Bombchu);
    InventoryItemsMenu.items[(u32)game::ItemId::DekuStick - 1].on = game::HasItem(game::ItemId::DekuStick);
    InventoryItemsMenu.items[(u32)game::ItemId::DekuNuts - 1].on = game::HasItem(game::ItemId::DekuNuts);
    InventoryItemsMenu.items[(u32)game::ItemId::MagicBean - 1].on = game::HasItem(game::ItemId::MagicBean);
    // InventoryItemsMenu.items[(u32)game::ItemId::FairySlingshot].on =
    // game::HasItem(game::ItemId::FairySlingshot);
    InventoryItemsMenu.items[(u32)game::ItemId::PowderKeg - 2].on = game::HasItem(game::ItemId::PowderKeg);
    InventoryItemsMenu.items[(u32)game::ItemId::PictographBox - 2].on = game::HasItem(game::ItemId::PictographBox);
    InventoryItemsMenu.items[(u32)game::ItemId::LensOfTruth - 2].on = game::HasItem(game::ItemId::LensOfTruth);
    InventoryItemsMenu.items[(u32)game::ItemId::Hookshot - 2].on = game::HasItem(game::ItemId::Hookshot);
    InventoryItemsMenu.items[(u32)game::ItemId::GreatFairySword - 2].on = game::HasItem(game::ItemId::GreatFairySword);
  ...
  static void Inventory_SongsMenuInit(void) {
    ...
    InventorySongsMenu.items[11].on = inventory.collect_register.scarecrows_song_icon;
  ...
  void Inventory_ItemsToggle(s32 selected) {
    ...
    switch (selected) {
    case ((s32)game::ItemId::Arrow):
      if (!game::HasItem(game::ItemId::Arrow)) {
        inventory.inventory_count_register.quiver_upgrade = game::Quiver::Quiver30;
        game::GiveItem(game::ItemId::Arrow);
        InventoryItemsMenu.items[selected].on = 1;
      } else {
        InventoryItemsMenu.items[selected].on = 0;
        game::RemoveItem(game::ItemId::Arrow);
        RemoveItemFromButtons(game::ItemId::Arrow);
        inventory.inventory_count_register.quiver_upgrade = game::Quiver::NoQuiver;
      }
  ...
  static void Inventory_BottlesMenuInit(void) {
    ...
    for (u32 i = (u32)game::ItemId::Bottle; i < (u32)game::ItemId::MoonTear; ++i) {
      InventoryBottlesMenu.items[curIndex].on = (i == (u32)SelectedBottle);
      ++curIndex;
    }
    // The last row is "None", which has no ItemId and so is not covered by the
    // loop above. Without this it never gets cleared once selected, and an empty
    // slot shows nothing selected at all.
    InventoryBottlesMenu.items[curIndex].on = !game::ItemIsBottled(SelectedBottle);
  }
  ...
  void Inventory_SongsToggle(s32 selected) {
    ...
    case (10):
      ...
      break;
    case (11):
      song_list.scarecrows_song_icon = !song_list.scarecrows_song_icon;
      break;
    case (12):
  ...
  void Inventory_ShieldsToggle(s32 selected) {
    ...
    // game::act::Player player = rnd::GetContext().GetPlayerActor();//gctx->GetPlayerActor();
  ...
  Menu InventoryMenu = {.title = "Inventory",
                        .nbItems = 6,
                        .items = {
  ...
  ToggleMenu InventoryItemsMenu = {.title = "Items",
                                   .nbItems = 22,
                                   .items = {
  ...
  ToggleMenu InventoryMasksMenu = {.title = "Masks",
                                   .nbItems = 24,
                                   .items = {
  ...
  ToggleMenu InventorySongsMenu = {
      ...
      .items = {{.on = 0, .title = "Sonata Of Awakening", .method = Inventory_SongsToggle},
                ...
                {.on = 0, .title = "Scarecrow Song icon (does nothing)", .method = Inventory_SongsToggle},
  ...
  ToggleMenu InventorySwordsMenu = {.title = "Swords",
                                    .nbItems = 4,
  ...
  ToggleMenu InventoryShieldsMenu = {.title = "Swords",
                                     .nbItems = 3,
                                     .items = {
  ...
  ToggleMenu InventoryBottlesMenu = {.title = "Choose Bottle Contents",
                                     .nbItems = 23,
                                     .items = {
