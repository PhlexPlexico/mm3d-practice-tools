...
void WatchesMenuFunc(void) {
  ...
  do {
    ...
    Draw_DrawFormattedString(30, 30 + WATCHES_MAX * SPACING_Y, COLOR_WHITE,
                             "Link position + angle: %-3s", showLinkWatches ? "on" : "off");
    Draw_DrawCharacter(10, 30 + WATCHES_MAX * SPACING_Y, COLOR_TITLE,
                       selected == (s32)WATCHES_MAX ? '>' : ' ');
...
static void drawLinkWatches(void) {
  ...
  Draw_DrawFormattedString(10, LINK_WATCH_Y, COLOR_WHITE, "X:%09.2F Y:%09.2F Z:%09.2F",
                           link->pos.pos.x, link->pos.pos.y, link->pos.pos.z);
  Draw_DrawFormattedString(10, LINK_WATCH_Y + SPACING_Y, COLOR_WHITE, "Angle:%6d      ",
                           link->actor_shape.rot.y);
}
