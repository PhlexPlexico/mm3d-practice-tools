...
u32 waitInputWithTimeout(u32 msec) {
  ...
  const bool isDPadPressed = (HID_PAD & BUTTON_UP) != 0 || (HID_PAD & BUTTON_DOWN) != 0 ||
                             (HID_PAD & BUTTON_LEFT) != 0 || (HID_PAD & BUTTON_RIGHT) != 0;
...
MyThread* menuCreateThread(void) {
  if (R_FAILED(
          MyThread_Create(&menuThread, menuThreadMain, menuThreadStack, 0x3000, 52, CORE_DEFAULT)))
...
static void menuDraw(Menu* menu, u32 selected) {
  ...
  Draw_DrawFormattedString(SCREEN_BOT_WIDTH - 10 - 4 * SPACING_X, SCREEN_BOT_HEIGHT - 20,
                           COLOR_WHITE, "    ");
...
void ToggleMenuShow(ToggleMenu* menu)  // displays a toggle menu, analogous to rosalina cheats page
{
  ...
  do {
    ...
    for (s32 i = 0;
         i < TOGGLE_MENU_MAX_SHOW && (u32)page * TOGGLE_MENU_MAX_SHOW + i < menu->nbItems; ++i) {
      s32 j = page * TOGGLE_MENU_MAX_SHOW + i;
      const char* checkbox = (menu->items[j].on ? "(x) " : "( ) ");
      Draw_DrawFormattedString(30, 30 + i * SPACING_Y, COLOR_WHITE, "%s%s", checkbox,
                               menu->items[j].title);
...
void AmountMenuShow(AmountMenu* menu) {  // displays an amount menu TODO: seems messed up
  ...
  do {
    ...
    for (s32 i = 0;
         i < AMOUNT_MENU_MAX_SHOW && (u32)page * AMOUNT_MENU_MAX_SHOW + i < menu->nbItems; ++i) {
...
u32 KeyboardFill(char* buf, u32 len) {
  ...
  do {
    ...
    for (u32 i = 0; i < 10; ++i) {
      Draw_DrawFormattedString(30 + (i * 2 * SPACING_X), 30,
                               (u32)selected == i ? COLOR_GREEN : COLOR_WHITE, "%c ", keys[i]);
    }
    for (u32 i = 0; i < 10; ++i) {
      u32 j = 10 + i;
    ...
    Draw_DrawString(10, SCREEN_BOT_HEIGHT - 20, COLOR_TITLE,
                    "L/R to switch case. Start to save and return.");
