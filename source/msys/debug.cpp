...
static s32 PopulateActorList(ShowActor_Info* list, game::act::Type type){
    s32 i = 0;
    rst::Context context = rst::GetContext();
    game::act::Player* player = context.gctx->GetPlayerActor();
    rst::ActorHeapNode* cur = (rst::ActorHeapNode*)((u32)player - sizeof(rst::ActorHeapNode));
    while (cur){
        if (!cur->free){
            game::act::Actor* actor = (game::act::Actor*)((u32)cur + sizeof(rst::ActorHeapNode));
...
static void DebugActors_ShowMoreInfo(game::act::Actor* actor) {
    ...
    do
    {
        ...
        Draw_DrawFormattedString(30, 30 + SPACING_Y, COLOR_WHITE, "Type:            %s", ActorTypeNames[(s32)actor->actor_type]);
        Draw_DrawFormattedString(30, 30 + 2 * SPACING_Y, COLOR_WHITE, "Params:          %04X", actor->params & 0xFFFF);
        Draw_DrawFormattedString(30, 30 + 3 * SPACING_Y, COLOR_WHITE, "Pos:             x:%05.2f  y:%05.2f  z:%05.2f", actor->pos.pos.x, actor->pos.pos.y, actor->pos.pos.z);
        Draw_DrawFormattedString(30, 30 + 4 * SPACING_Y, COLOR_WHITE, "Rot:             x:%04X  y:%04X  z:%04X", actor->pos.rot.x & 0xFFFF, actor->pos.rot.y & 0xFFFF, actor->pos.rot.z & 0xFFFF);
        Draw_DrawFormattedString(30, 30 + 5 * SPACING_Y, COLOR_WHITE, "Vel:             x:%05.2f  y:%05.2f  z:%05.2f", actor->vel.x, actor->vel.y, actor->vel.z);
        //Draw_DrawFormattedString(30, 30 + 6 * SPACING_Y, COLOR_WHITE, "Floor:           %08X", actor->floorPoly);
        Draw_DrawFormattedString(30, 30 + 7 * SPACING_Y, COLOR_WHITE, "Dist. from Link: xz:%05.2f  y:%05.2f", actor->distance_to_link, actor->height_diff_to_link);
        Draw_DrawFormattedString(30, 30 + 8 * SPACING_Y, COLOR_WHITE, "Text ID:         %04X", actor->text_id_maybe & 0xFFFF);
        Draw_DrawFormattedString(30, 30 + 9 * SPACING_Y, COLOR_WHITE, "Held By:         %08X", actor->child_actor);
        Draw_DrawFormattedString(30, 30 + 10 * SPACING_Y, COLOR_WHITE, "Holding:         %08X", actor->parent_actor);
...
static void DebugActors_ShowActors(void) {
    ...
    do
    {
        ...
        for (s32 i = 0; i < ACTOR_LIST_MAX_SHOW && page * ACTOR_LIST_MAX_SHOW + i < listSize; ++i)
        {
            ...
            Draw_DrawFormattedString(30, 30 + (1 + i) * SPACING_Y, COLOR_WHITE, "Addr: %08X  ID: %04X", actorList[j].instance, actorList[j].id);
