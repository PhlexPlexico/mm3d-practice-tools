...
namespace msys {
  Menu WarpsMenu = {
      ...
      .items = {{.title = "Places", .action_type = METHOD, .method = WarpsPlacesMenuShow},
                ...
                {.title = "Owl Statues", .action_type = METHOD, .method = Warps_OwlStatuesMenuFunc}}};

  WarpsSceneMenu WarpsPlacesOverworldSwampMenu = {.title = "Swamp Overworld",
                                                  .nbItems = 12,
                                                  .scenes = {
  ...
  WarpsSceneMenu WarpsPlacesOverworldIkanaMenu = {
      ...
      .nbItems = 15,
      .scenes = {{"0A: Beneath The Graveyard Iron Knuckles", &Entrances_Beneath_The_Graveyard_Iron_Knuckles},
                 {"14: Grottos", &Entrances_Lone_Peak_Shrine_And_Grottos},
  ...
  WarpsSceneMenu WarpsPlacesOverworldTerminaMenu = {.title = "Termina Overworld",
                                                    .nbItems = 7,
  ...
  WarpsSceneMenu WarpsPlacesTownsMenu = {.title = "Clock Town Entrances",
                                         .nbItems = 19,
                                         .scenes = {
  ...
  WarpsSceneMenu WarpsPlacesBossesMenu = {.title = "Boss Rooms",
                                          .nbItems = 6,
                                          .scenes = {
  ...
  WarpsSceneMenu WarpsPlacesShopsMenu = {.title = "Shops",
                                         .nbItems = 9,
                                         .scenes = {
  ...
  WarpsSceneMenu WarpsPlacesMiscMenu = {.title = "Misc Places/Cutscenes",
                                        .nbItems = 6,
                                        .scenes = {
  ...
  WarpsSceneMenu WarpsSpiderHouses = {.title = "Spider Houses",
                                      .nbItems = 2,
                                      .scenes = {
  ...
  ToggleMenu WarpsOwlStatuesMenu = {.title = "Owl Statues",
                                    .nbItems = 10,
                                    .items = {
                                        {.on = 0, .title = "Great Bay Coast", .method = Warps_OwlStatuesToggle},
                                        {.on = 0, .title = "Zora Cape", .method = Warps_OwlStatuesToggle},
                                        {.on = 0, .title = "Snowhead", .method = Warps_OwlStatuesToggle},
                                        {.on = 0, .title = "Mountain Village", .method = Warps_OwlStatuesToggle},
                                        {.on = 0, .title = "Clock Town", .method = Warps_OwlStatuesToggle},
                                        {.on = 0, .title = "Milk Road", .method = Warps_OwlStatuesToggle},
                                        {.on = 0, .title = "Woodfall", .method = Warps_OwlStatuesToggle},
                                        {.on = 0, .title = "Southern Swamp", .method = Warps_OwlStatuesToggle},
                                        {.on = 0, .title = "Ikana Canyon", .method = Warps_OwlStatuesToggle},
                                        {.on = 0, .title = "Stone Tower", .method = Warps_OwlStatuesToggle},
                                    }};
  ...
  void ManuallyEnterEntranceIndex(void) {
    ...
    do {
      ...
      Draw_DrawFormattedString(30, 30 + SPACING_Y * Manual_Entrance_Menu_EtcIdx, COLOR_WHITE, "Entrance Index: 0x%04X",
                               chosenIndex);
      ...
      if (pressed & BUTTON_B && !chosen)
        ...
        selected++;
      } else if (pressed & MENU_DOWN && chosen && selected == Manual_Entrance_Menu_EtcIdx) {
        chosenIndex--;
      } else if (pressed & MENU_UP && !chosen) {
        selected--;
      } else if (pressed & MENU_UP && chosen && selected == Manual_Entrance_Menu_EtcIdx) {
        chosenIndex++;
      } else if (pressed & MENU_LEFT && chosen && selected == Manual_Entrance_Menu_EtcIdx) {
        chosenIndex -= 16;
      } else if (pressed & MENU_RIGHT && chosen && selected == Manual_Entrance_Menu_EtcIdx) {
        chosenIndex += 16;
  ...
  void Warps_OwlStatuesToggle(s32 selected) {
    game::PlayerData::OwlStatues& owls = game::GetCommonData().save.player.owl_statue_flags;
    owls.raw ^= (u16)(1u << selected);
    WarpsOwlStatuesMenu.items[selected].on = (owls.raw >> selected) & 1;
  }

  static void Warps_OwlStatuesMenuInit(void) {
    const u16 raw = game::GetCommonData().save.player.owl_statue_flags.raw;
    for (u32 i = 0; i < WarpsOwlStatuesMenu.nbItems; ++i)
      WarpsOwlStatuesMenu.items[i].on = (raw >> i) & 1;
  }

  void Warps_OwlStatuesMenuFunc(void) {
    Warps_OwlStatuesMenuInit();
    ToggleMenuShow(&WarpsOwlStatuesMenu);
  }
