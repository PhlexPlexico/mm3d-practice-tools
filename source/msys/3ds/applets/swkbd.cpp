...
#include "../../include/3ds/applets/swkbd.h"
#include "../../include/3ds/ipc.h"
#include "../../include/3ds/env.h"
#include "../../include/3ds/util/utf.h"
...
{
	int i;
	for (i = 0; i < (int)(sizeof(swkbdFeatures)/sizeof(u16)); i ++)
...
static void swkbdMessageCallback(void* user, NS_APPID sender, void* msg, size_t msgsize)
{
	...
	free(text8);
	APT_SendParameter((NS_APPID)envGetAptAppId(), sender, APTCMD_MESSAGE, swkbd, sizeof(*swkbd), 0);
}
...
SwkbdButton swkbdInputText(SwkbdState* swkbd, char* buf, size_t bufsize)
{
	...
	Result res = svcCreateMemoryBlock(&swkbdSharedMemHandle, (u32)swkbdSharedMem, sharedMemSize, (MemPerm)(MEMPERM_READ|MEMPERM_WRITE), (MemPerm)(MEMPERM_READ|MEMPERM_WRITE));
	if (R_FAILED(res))
	{
		free(swkbdSharedMem);
		swkbd->result = SWKBD_OUTOFMEM;
