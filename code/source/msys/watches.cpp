...
namespace msys {
  ...
  static void WatchesEditWatch(s32 selected) {
    ...
    do {
      ...
      Draw_DrawFormattedString(30, 30 + 2 * SPACING_Y, COLOR_WHITE, "Addr: %02X %02X %02X %02X", bytes[0] & 0xFF,
                               bytes[1] & 0xFF, bytes[2] & 0xFF, bytes[3] & 0xFF);
      if (selectedItem == 2) {
        Draw_DrawFormattedString(30 + (6 * SPACING_X) + (3 * SPACING_X * selectedByte), 30 + 2 * SPACING_Y,
                                 chosen ? COLOR_RED : COLOR_GREEN, "%02X", bytes[selectedByte] & 0xFF);
      }
      ...
      if (chosen) {
        ...
        if (pressed & BUTTON_B)
          ...
          selectedItem++;
        else if (selectedItem == 2 && pressed & MENU_LEFT)
          selectedByte--;
        else if (selectedItem == 2 && pressed & MENU_RIGHT)
          selectedByte++;
      }
  ...
  void WatchesMenuFunc(void) {
    ...
    do {
      ...
      Draw_DrawFormattedString(30, 30 + WATCHES_MAX * SPACING_Y, COLOR_WHITE, "Link position + angle: %-3s",
                               showLinkWatches ? "on" : "off");
      Draw_DrawCharacter(10, 30 + WATCHES_MAX * SPACING_Y, COLOR_TITLE, selected == (s32)WATCHES_MAX ? '>' : ' ');
      ...
      if (pressed & BUTTON_B)
        ...
        // The row past the user watches is the built-in Link readout.
        if (selected == (s32)WATCHES_MAX) {
          Watches_ToggleLinkWatches();
          continue;
        }
      ...
      // One past the user watches is the built-in Link readout.
      if (selected > (s32)WATCHES_MAX)
        selected = 0;
      if (selected < 0)
        selected = WATCHES_MAX;
  ...
  /*
   * Link's position and facing, always available without having to look up an
   * address. They cannot be ordinary watches: those hold a fixed address, and the
   * player actor moves, so the pointer has to be resolved every frame.
   *
   * Facing is actor_shape.rot.y -- the same field save states store as "angle".
   */
  bool showLinkWatches = false;

/*
 * Two lines, not four: at four it ran past the bottom of the screen. The last
 * user watch ends at y=204, so this sits below that and still clears 240.
 */
#define LINK_WATCH_Y (SCREEN_BOT_HEIGHT - 2 * SPACING_Y - 4)

  static void drawLinkWatches(void) {
    game::GlobalContext* gctx = rnd::GetContext().gctx;
    game::act::Player* link = gctx ? gctx->GetPlayerActor() : nullptr;
    if (!link) {
      Draw_DrawFormattedString(10, LINK_WATCH_Y, COLOR_WHITE, "%-42s", "Link: not loaded");
      Draw_DrawFormattedString(10, LINK_WATCH_Y + SPACING_Y, COLOR_WHITE, "%-42s", "");
      return;
    }
    Draw_DrawFormattedString(10, LINK_WATCH_Y, COLOR_WHITE, "X:%09.2F Y:%09.2F Z:%09.2F", link->pos.pos.x,
                             link->pos.pos.y, link->pos.pos.z);
    Draw_DrawFormattedString(10, LINK_WATCH_Y + SPACING_Y, COLOR_WHITE, "Angle:%6d      ", link->actor_shape.rot.y);
  }

  void Watches_ToggleLinkWatches(void) {
    showLinkWatches = !showLinkWatches;
  }
  ...
  void Watches_Init() {
    if (msys::File_CheckOrCreateProfileDirectory()) {
      // Name-keyed binary blob; see File_LoadWatches. The JSON version iterated
      // the object and trusted every field, including the address.
      msys::File_LoadWatches(watches);
    }
  }
}  // namespace msys
