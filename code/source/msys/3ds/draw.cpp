...
// Bounded, and keeps newlib's full printf out of the image.
...
void Draw_DrawCharacter(u32 posX, u32 posY, u32 color, char character)
{
    ...
    // screenPos is unsigned and the row term is (HEIGHT - y - posY - 1), so a
    // glyph running past the bottom does not clip -- it underflows and borrows
    // from the previous column, drawing somewhere else entirely.
    if(posY + 10 > SCREEN_BOT_HEIGHT || posX + 6 > SCREEN_BOT_WIDTH) return;
...
void Draw_DrawCharacterTop(u32 posX, u32 posY, u32 color, char character)
{
    ...
    if(posY + 10 > SCREEN_TOP_HEIGHT || posX + 6 > SCREEN_TOP_WIDTH) return;
...
{
    Graphics* graphics = *rnd::util::GetPointer<Graphics*>(ADDR_Draw_SetupFramebuffer_6A3A4C);
