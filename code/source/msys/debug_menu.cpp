...
namespace msys {
  ...
  static const char* const ActorTypeNames[] = {
      "SWITCH",  // 0x0
      "BG",     "PLAYER", "EXPLOSIVES", "NPC", "ENEMY", "PROP", "ITEMACTION", "MISC", "BOSS", "DOOR", "CHEST",
      "ALL",  // 0xC
  };
  ...
  static void DebugActors_ShowMoreInfo(game::act::Actor* actor) {
    rnd::Context context = rnd::GetContext();
    ...
    do {
      ...
      Draw_DrawFormattedString(30, 30 + 3 * SPACING_Y, COLOR_WHITE, "Pos:             x:%05.2f  y:%05.2f  z:%05.2f",
                               actor->pos.pos.x, actor->pos.pos.y, actor->pos.pos.z);
      Draw_DrawFormattedString(30, 30 + 4 * SPACING_Y, COLOR_WHITE, "Rot:             x:%04X  y:%04X  z:%04X",
                               actor->pos.rot.x & 0xFFFF, actor->pos.rot.y & 0xFFFF, actor->pos.rot.z & 0xFFFF);
      Draw_DrawFormattedString(30, 30 + 5 * SPACING_Y, COLOR_WHITE, "Vel:             x:%05.2f  y:%05.2f  z:%05.2f",
                               actor->vel.x, actor->vel.y, actor->vel.z);
      // Draw_DrawFormattedString(30, 30 + 6 * SPACING_Y, COLOR_WHITE, "Floor:           %08X",
      // actor->floorPoly);
      Draw_DrawFormattedString(30, 30 + 7 * SPACING_Y, COLOR_WHITE, "Dist. from Link: xz:%05.2f  y:%05.2f",
                               actor->distance_to_link, actor->height_diff_to_link);
