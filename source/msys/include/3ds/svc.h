...
typedef enum {
  ...
  ARBITRATION_DECREMENT_AND_WAIT_IF_LESS_THAN =
      2,  ///< If the memory at the address is strictly lower than #value, then decrement it and
          ///< wait for signal.
  ARBITRATION_WAIT_IF_LESS_THAN_TIMEOUT = 3,  ///< If the memory at the address is strictly lower
                                              ///< than #value, then wait for signal or timeout.
  ARBITRATION_DECREMENT_AND_WAIT_IF_LESS_THAN_TIMEOUT =
      4,  ///< If the memory at the address is strictly lower than #value, then decrement it and
...
typedef enum {
  RESET_ONESHOT = 0,  ///< When the primitive is signaled, it will wake up exactly one thread and
                      ///< will clear itself automatically.
  RESET_STICKY = 1,   ///< When the primitive is signaled, it will wake up all threads and it won't
                      ///< clear itself automatically.
  RESET_PULSE = 2,  ///< Only meaningful for timers: same as ONESHOT but it will periodically signal
                    ///< the timer instead of just once.
} ResetType;
...
typedef enum {
  ...
  EXITPROCESS_EVENT_DEBUG_TERMINATE =
      2,  ///< Process has been terminated by @ref svcTerminateDebugProcess.
...
typedef enum {
  ...
  EXITTHREAD_EVENT_EXIT_PROCESS =
      2,  ///< Process exited either normally or due to an uncaught exception.
  EXITTHREAD_EVENT_TERMINATE_PROCESS =
      3,  ///< Process has been terminated by @ref svcTerminateProcess.
...
/// Event relating to fault exceptions (CPU exceptions other than stop points and undefined
/// syscalls).
typedef struct {
...
typedef struct {
  s32 thread_ids[4];  ///< IDs of the attached process's threads that were running on each core at
                      ///< the time of the @ref svcBreakDebugProcess call, or -1 (only the first 2
                      ///< values are meaningful on O3DS).
...
typedef struct {
  ...
  u32 flags;  ///< Flags. Bit0 means that @ref svcContinueDebugEvent needs to be called for this
              ///< event (except for EXIT PROCESS events, where this flag is disregarded).
...
typedef enum {
  DBG_INHIBIT_USER_CPU_EXCEPTION_HANDLERS =
      BIT(0),  ///< Inhibit user-defined CPU exception handlers (including watchpoints and
               ///< breakpoints, regardless of any @ref svcKernelSetState call).
  DBG_SIGNAL_FAULT_EXCEPTION_EVENTS =
      BIT(1),  ///< Signal fault exception events. See @ref FaultExceptionEvent.
  DBG_SIGNAL_SCHEDULE_EVENTS =
      BIT(2),  ///< Signal schedule in/out events. See @ref ScheduleInOutEvent.
  DBG_SIGNAL_SYSCALL_EVENTS =
      BIT(3),                      ///< Signal syscall in/out events. See @ref SyscallInOutEvent.
 ...
 * @param addr1    The virtual address to be (un)mirrored by @p addr0 when using @ref MEMOP_MAP or
 * @ref MEMOP_UNMAP. It has to be pointing to a RW memory. Use NULL if the operation is @ref
 * MEMOP_FREE or @ref MEMOP_ALLOC.
 * @param size     The requested size for @ref MEMOP_ALLOC and @ref MEMOP_ALLOC_LINEAR.
 * @param op       Operation flags. See @ref MemOp.
 * @param perm     A combination of @ref MEMPERM_READ and @ref MEMPERM_WRITE. Using MEMPERM_EXECUTE
 * will return an error. Value 0 is used when unmapping memory.
 *
 * If a memory is mapped for two or more addresses, you have to use MEMOP_UNMAP before being able to
 * MEMOP_FREE it. MEMOP_MAP will fail if @p addr1 was already mapped to another address.
 *
 ...
 */
Result svcCreateMemoryBlock(Handle* memblock, u32 addr, u32 size, MemPerm my_perm,
                            MemPerm other_perm);
 ...
 */
Result svcStartInterProcessDma(Handle* dma, Handle dstProcess, void* dst, Handle srcProcess,
                               const void* src, u32 size, void* dmaConfig);
 ...
 */
Result svcCreateCodeSet(Handle* out, const CodeSetInfo* info, void* code_ptr, void* ro_ptr,
                        void* data_ptr);
 ...
 */
Result svcCreateProcess(Handle* out, Handle codeset, const u32* arm11kernelcaps,
                        u32 arm11kernelcaps_num);
 ...
 * @param processor_id    The id of the processor the thread should be ran on. Those are labelled
 * starting from 0. For old 3ds it has to be <2, and for new 3DS <4. Value -1 means all CPUs and -2
 * read from the Exheader.
 *
 * The processor with ID 1 is the system processor.
 * To enable multi-threading on this core you need to call APT_SetAppCpuTimeLimit at least once with
 * a non-zero value.
 *
 ...
 *
 * @note The kernel will clear the @p stack_top's address low 3 bits to make sure it is 0x8-bytes
 * aligned.
 */
Result svcCreateThread(Handle* thread, ThreadFunc entrypoint, u32 arg, u32* stack_top,
                       s32 thread_priority, s32 processor_id);
 ...
 * It means that you can join a thread by calling @code
 * svcWaitSynchronization(threadHandle,yourtimeout); @endcode
 */
 ...
 */
Result svcGetResourceLimitLimitValues(s64* values, Handle resourceLimit, ResourceLimitType* names,
                                      s32 nameCount);
 ...
 * @param nameCount Number of resource limit names.
 */
Result svcSetResourceLimitValues(Handle resourceLimit, const ResourceLimitType* names,
                                 const s64* values, s32 nameCount);
 ...
 * This requests always return an error when called, it only checks if the handle is a thread or
 * not.
 ...
 * @param nanoseconds Maximum nanoseconds to wait for.
 */
Result svcWaitSynchronizationN(s32* out, const Handle* handles, s32 handles_num, bool wait_all,
                               s64 nanoseconds);
 ...
 * @param value Number of threads to signal if using @ref ARBITRATION_SIGNAL, or the value used for
 * comparison.
 *
 * This will perform an arbitration based on #type. The comparisons are done between #value and the
 * value at the address #addr.
 *
 * @code
 * s32 val=0;
 ...
 * @endcode
 */
Result svcArbitrateAddress(Handle arbiter, u32 addr, ArbitrationType type, s32 value,
                           s64 nanoseconds);
 ...
 */
void svcBreakRO(UserBreakType breakReason, const void* croInfo,
                u32 croInfoSize) __asm__("svcBreak");
...
/**
 * @brief Fetches the saved registers of a thread, either inactive or awaiting @ref
...
/**
 * @brief Updates the saved registers of a thread, either inactive or awaiting @ref
 * svcContinueDebugEvent, belonging to a debugged process.
 * @param debug Debug handle of the parent process.
...
/**
 * @brief Sets an hardware breakpoint or watchpoint. This is an interface to the BRP/WRP registers,
 * see http://infocenter.arm.com/help/topic/com.arm.doc.ddi0360f/CEGEBGFC.html .
 * @param registerId range 0..5 = breakpoints (BRP0-5), 0x100..0x101 = watchpoints (WRP0-1). The
 * previous stop point for the register is disabled.
 * @param control Value of the control regiser.
 * @param value Value of the value register: either and address (if bit21 of control is clear) or
 * the debug handle of a process to fetch the context ID of.
 */
 ...
 */
Result svcGetDebugThreadParam(s64* unused, u32* out, Handle debug, u32 threadId,
