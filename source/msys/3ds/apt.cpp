...
#include "../include/3ds/services/apt.h"
#include "../include/3ds/services/gspgpu.h"
#include "../include/3ds/services/ptmsysm.h" // for PtmWakeEvents
#include "../include/3ds/allocator/mappable.h"
#include "../include/3ds/ipc.h"
#include "../include/3ds/env.h"
#include "../include/3ds/thread.h"
#include "../include/3ds/os.h"
...
Result aptInit(void)
{
	...
	ret = APT_Initialize((NS_APPID)envGetAptAppId(), attr, &aptEvents[0], &aptEvents[1]);
	if (R_FAILED(ret)) {
...
void aptSetSleepAllowed(bool allowed)
{
	...
	{
		aptFlags &= ~FLAG_ALLOWSLEEP;
		APT_ReplySleepQuery((NS_APPID)envGetAptAppId(), APTREPLY_REJECT);
	}
}
...
void aptExit(void)
{
	...
	if (!aptIsCrippled())
	{
		...
		else if (aptIsReinit())
		{
			...
			APT_Finalize((NS_APPID)envGetAptAppId());
			doClose = false;
		}
...
void aptEventHandler(void *arg)
{
	while (!aptEventHandlerThreadQuit)
	{
		...
		if (id == 1)
		{
			NS_APPID sender;
			APT_Command cmd;
			Result res = APT_GlanceParameter((NS_APPID)envGetAptAppId(), aptParameters, sizeof(aptParameters), &sender, &cmd, NULL, NULL);
			if (R_FAILED(res))
		...
		APT_Signal signal;
		Result res = APT_InquireNotification((NS_APPID)envGetAptAppId(), &signal);
		if (R_FAILED(res))
			continue;
		...
		switch (signal)
		{
			...
			case APTSIGNAL_SLEEP_QUERY:
			{
				...
				_aptDebug(11, reply);
				APT_ReplySleepQuery((NS_APPID)envGetAptAppId(), reply);
				break;
			}
			...
			case APTSIGNAL_SLEEP_ENTER:
				...
				if (aptIsActive())
					...
					APT_ReplySleepNotificationComplete((NS_APPID)envGetAptAppId());
				break;
...
static Result aptReceiveParameter(APT_Command* cmd, size_t* actualSize, Handle* handle)
{
	...
	Result res = APT_ReceiveParameter((NS_APPID)envGetAptAppId(), aptParameters, sizeof(aptParameters), &sender, cmd, actualSize, handle);
...
{
	APT_Command cmd;
	APT_NotifyToWait((NS_APPID)envGetAptAppId());
...
static void aptScreenTransfer(NS_APPID appId, bool sysApplet)
{
	...
	for (;;)
	{
		Result res = APT_SendParameter((NS_APPID)envGetAptAppId(), appId, sysApplet ? APTCMD_SYSAPPLET_REQUEST : APTCMD_REQUEST, &capinfo, sizeof(capinfo), 0);
...
Result APT_GetAppletManInfo(APT_AppletPos inpos, APT_AppletPos* outpos, NS_APPID* req_appid, NS_APPID* menu_appid, NS_APPID* active_appid)
{
	...
	if (R_SUCCEEDED(ret))
	{
		if (outpos)       *outpos      =(APT_AppletPos)cmdbuf[2];
		if (req_appid)    *req_appid   =(NS_APPID)cmdbuf[3];
		if (menu_appid)   *menu_appid  =(NS_APPID)cmdbuf[4];
		if (active_appid) *active_appid=(NS_APPID)cmdbuf[5];
	}
...
Result APT_InquireNotification(u32 appID, APT_Signal* signalType)
{
	...
	Result ret = aptSendCommand(cmdbuf);
	if (R_SUCCEEDED(ret))
		*signalType=(APT_Signal)cmdbuf[2];
