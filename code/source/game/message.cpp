...
namespace game {
  ...
  MessageMgr& MessageMgr::Instance() {
    return rnd::util::GetPointer<MessageMgr&()>(ADDR_Instance_1C51D0)();
  }
  ...
  bool MessageData::Get(u32 id, Message* msg) const {
    ...
    for (size_t i = 0, res_idx = 0; i < NumLanguages; ++i) {
      ...
      text.reader = rnd::util::GetPointer<MessageReader*(Language)>(ADDR_if_1C519C)(Language(i));
...
// entry->texts[res_idx].length);
  ...
  void PlayMessagePassSound() {
    rnd::util::GetPointer<void()>(ADDR_PlayMessagePassSound_1AF034)();
  }

  void PlayMessageCancelSound() {
    rnd::util::GetPointer<void()>(ADDR_PlayMessageCancelSound_1AE498)();
  }
