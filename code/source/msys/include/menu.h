...
// Button constants and the pad read both come from hid.h / common/input.h, so
// the menu sees ZL, ZR, the C-stick and the circle pad like the rest of the
// code does. Raw shared HID reports none of those.
#define HID_PAD (rnd::GetCurrentPadState())

// Menu navigation takes the circle pad as well as the D-pad. Deliberately not
// folded into BUTTON_* : command bindings have to keep telling them apart, or
// Frame Advance on D-pad Up would also fire every time the stick is pushed up.
#define MENU_UP (BUTTON_UP | CPAD_UP)
#define MENU_DOWN (BUTTON_DOWN | CPAD_DOWN)
#define MENU_LEFT (BUTTON_LEFT | CPAD_LEFT)
#define MENU_RIGHT (BUTTON_RIGHT | CPAD_RIGHT)
